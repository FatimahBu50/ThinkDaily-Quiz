    ...
    public boolean addPlayer(String username, String password, String userDisplayName,
        ...
        try {
         ...
         //  Duplicate username (unique constraint)
          System.out.println("Signup blocked: username already exists -> " + username);
          return false;

        } catch (SQLException e) {
        //  Other DB errors (keep a short log, no stacktrace spam)
        System.out.println("DB error in addPlayer: " + e.getMessage());
        return false;
        }
...
if (categoryName == null || categoryName.isBlank() || "Random".equalsIgnoreCase(categoryName)) {
    ...
    q = """
        ...
        while (rs.next()) {
            ...
            String text = rs.getString("question_text");
            String diff = rs.getString("difficulty");
            String catNameFromDb = rs.getString("category_name"); // ✅ THIS
            int catIdFromDb = rs.getInt("category_id");   
    ...
    qid,
    text,
    catNameFromDb,       //  REAL category (Math / Science / Technology)
    options,
    correctIndex,
    diff,                // keep real difficulty
    true
);

qm.categoryId = catIdFromDb;
...
public boolean addPointsToUser(int userId, int points) throws SQLException {
    ...
    String updatePoints = """
        UPDATE users
        SET current_points_total = current_points_total + ?
        WHERE user_id = ?
    """;

    PreparedStatement ps1 = conn.prepareStatement(updatePoints);
    ps1.setInt(1, points);
    ps1.setInt(2, userId);
    ps1.executeUpdate();
    ...
    String updateLevel = """
        UPDATE users
        SET current_level = (current_points_total DIV 100) + 1
        WHERE user_id = ?
    """;

    PreparedStatement ps2 = conn.prepareStatement(updateLevel);
    ps2.setInt(1, userId);
    ps2.executeUpdate();
