...
public class Leaderboard_Moniter extends javax.swing.JPanel {
    ...
    private void loadLeaderboardData(String timePeriod, String category) {
        ...
        try {
            ...
            if ("Today".equals(timePeriod)) {
                ...
                leaderboardData = getWeeklyLeaderboard(category);
            } else if ("This Month".equals(timePeriod)) {
                leaderboardData = getMonthlyLeaderboard(category);
            } else {
    ...
    private List<LeaderboardEntry> getDailyLeaderboard(String category) throws SQLException {
        ...
        String sql = """
            ...
            GROUP BY u.user_id, u.user_name, u.current_level
    ...
    private List<LeaderboardEntry> getWeeklyLeaderboard(String category) throws SQLException {
        List<LeaderboardEntry> entries = new ArrayList<>();
        
        String sql = """
            SELECT 
                u.user_name as username,
                COALESCE(SUM(ur.points_awarded), 0) as weekly_points,
                u.current_level as level,
                COALESCE(AVG(TIMESTAMPDIFF(SECOND, ur.answered_at, NOW())), 0) as response_time
            FROM users u
            JOIN accounts a ON u.account_id = a.account_id
            LEFT JOIN user_responses ur ON u.user_id = ur.user_id 
                AND YEARWEEK(ur.answered_at) = YEARWEEK(CURDATE())
                AND ur.is_correct = TRUE
            WHERE a.role = 'PLAYER' 
                AND a.is_active = TRUE
            GROUP BY u.user_id, u.user_name, u.current_level
            ORDER BY weekly_points DESC, response_time ASC
            LIMIT 10
            """;
        ...
        try {
            ...
            while (rs.next()) {
                LeaderboardEntry entry = new LeaderboardEntry();
                entry.username = rs.getString("username");
                entry.points = rs.getInt("weekly_points");
                entry.level = rs.getInt("level");
                entry.responseTime = rs.getDouble("response_time");
                entries.add(entry);
            }
    ...
    private List<LeaderboardEntry> getMonthlyLeaderboard(String category) throws SQLException {
        List<LeaderboardEntry> entries = new ArrayList<>();
        
        String sql = """
            SELECT 
                u.user_name as username,
                COALESCE(SUM(ur.points_awarded), 0) as monthly_points,
                u.current_level as level,
                COALESCE(AVG(TIMESTAMPDIFF(SECOND, ur.answered_at, NOW())), 0) as response_time
            FROM users u
            JOIN accounts a ON u.account_id = a.account_id
            LEFT JOIN user_responses ur ON u.user_id = ur.user_id 
                AND YEAR(ur.answered_at) = YEAR(CURDATE())
                AND MONTH(ur.answered_at) = MONTH(CURDATE())
                AND ur.is_correct = TRUE
            WHERE a.role = 'PLAYER' 
                AND a.is_active = TRUE
            GROUP BY u.user_id, u.user_name, u.current_level
            ORDER BY monthly_points DESC, response_time ASC
            LIMIT 10
            """;
        ...
        try {
            ...
            while (rs.next()) {
                LeaderboardEntry entry = new LeaderboardEntry();
                entry.username = rs.getString("username");
                entry.points = rs.getInt("monthly_points");
                entry.level = rs.getInt("level");
                entry.responseTime = rs.getDouble("response_time");
                entries.add(entry);
            }
    ...
    private List<LeaderboardEntry> getAllTimeLeaderboard(String category) throws SQLException {
        ...
        String sql = """
            ...
            WHERE a.role = 'PLAYER' 
                AND a.is_active = TRUE
            GROUP BY u.user_id, u.user_name, u.current_points_total, u.current_level
    ...
    private void initComponents() {
        ...
        jComboBox2.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { 
            "All Categories", "Science & Technology", "History & Geography", 
            "Sports & Entertainment", "Mathematics & Logic", "Language & Literature", "General Knowledge" 
        }));

        jComboBox1.setBackground(new java.awt.Color(255, 245, 250));
        jComboBox1.setFont(new java.awt.Font("SansSerif", Font.PLAIN, 12));
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Today", "This Week", "This Month", "All Time" }));
        ...
        // Update table model to include Level column
        ...
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                ...
                // Logo
                // ===== CENTER LOGO =====
// ===== LOGO: Centered but slightly left =====
.addGroup(layout.createSequentialGroup()
    .addGap(280)   // smaller left gap → moves slightly left
    .addComponent(jLabel4, GroupLayout.PREFERRED_SIZE, 120, GroupLayout.PREFERRED_SIZE)
    .addGap(0, 0, Short.MAX_VALUE)
)

// ===== TITLE: Centered but slightly left =====
.addGroup(layout.createSequentialGroup()
    .addGap(230)   // same adjustment
